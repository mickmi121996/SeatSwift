...
namespace GuichetAutonome.DataAccessLayer.Factories
{
    ...
    public class ShowFactory : Base.FactoryBase
    {
        ...
        /// </summary>
        /// <returns>A list of all active shows with incoming representation</returns>
        /// <exception cref="Exception">If an error occurs while executing the query</exception>
        /// <exception cref="KeyNotFoundException">If no show with incoming representation is found</exception>
        public async Task<List<Show>> GetAllActiveWithIncomingRepresentationAsync()
        {
            try
            {
                // Get all active shows with incoming representation
